...
@interface TVOnAirViewController ()


@end

@implementation TVOnAirViewController
@synthesize jsonURL, spinner;
...
- (void)viewDidLoad
{
    ...
    self.pOnAirArray = [[NSMutableArray alloc] init];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSNumber *movieID = [[_pOnAirArray objectAtIndex:indexPath.row] objectForKey:@"id"] ;
    ...
    movieDetail.fullInfoURL = [NSString stringWithFormat:@"http://api.themoviedb.org/3/tv/%i?api_key=c7dbad2bd6e90365dd9a822e201bf107&append_to_response=videos", mID];
    movieDetail.seasonID = mID;
    //poster setup
    NSString* posterPrefix = @"http://image.tmdb.org/t/p/w300";
    NSString* posterSuffix = [[_pOnAirArray objectAtIndex:indexPath.row] objectForKey:@"backdrop_path"];
         ...
         cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text = [[_pOnAirArray objectAtIndex:indexPath.row] objectForKey:@"original_name"];
    cell.textLabel.textColor = [self colorFromHexString:@"#FFFFFF"];
    cell.detailTextLabel.text = [NSString stringWithFormat:@"Release: %@", [[_pOnAirArray objectAtIndex:indexPath.row] objectForKey:@"first_air_date"]];
    cell.detailTextLabel.textColor = [self colorFromHexString:@"#595959"];
    
    //poster
    NSString* posterPrefix = @"http://image.tmdb.org/t/p/w92";
    NSString* posterSuffix = [[_pOnAirArray objectAtIndex:indexPath.row] objectForKey:@"poster_path"];
...
{
    int count = [_pOnAirArray count];
    return [_pOnAirArray count];
}


- (void)loadOnAirAsynchroniously
{
    ...
    [manager GET:@"http://api.themoviedb.org/3/tv/on_the_air?api_key=c7dbad2bd6e90365dd9a822e201bf107" parameters:nil
         success:^(AFHTTPRequestOperation *operation, id responseObject) {
             ...
             _pOnAirArray = [responseObject objectForKey:@"results"];
