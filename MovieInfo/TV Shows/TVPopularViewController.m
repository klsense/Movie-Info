...
#import "TVPopularViewController.h"
...
@interface TVPopularViewController ()

@end

@implementation TVPopularViewController
@synthesize spinner;
...
- (void)viewDidLoad
{
    ...
    self.pPopularArray = [[NSMutableArray alloc] init];
    ...
    [self loadPopularAsynchroniously];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSNumber *movieID = [[_pPopularArray objectAtIndex:indexPath.row] objectForKey:@"id"] ;
    ...
    NSString* posterSuffix = [[_pPopularArray objectAtIndex:indexPath.row] objectForKey:@"backdrop_path"];
         ...
         cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text = [[_pPopularArray objectAtIndex:indexPath.row] objectForKey:@"original_name"];
    cell.textLabel.textColor = [self colorFromHexString:@"#FFFFFF"];
    cell.detailTextLabel.text = [NSString stringWithFormat:@"Release: %@", [[_pPopularArray objectAtIndex:indexPath.row] objectForKey:@"first_air_date"]];
    cell.detailTextLabel.textColor = [self colorFromHexString:@"#595959"];
    
    //poster
    NSString* posterPrefix = @"http://image.tmdb.org/t/p/w92";
    NSString* posterSuffix = [[_pPopularArray objectAtIndex:indexPath.row] objectForKey:@"poster_path"];
...
{
    int count = [_pPopularArray count];
    return [_pPopularArray count];
}


- (void)loadPopularAsynchroniously
{
    ...
    [manager GET:@"http://api.themoviedb.org/3/tv/on_the_air?api_key=c7dbad2bd6e90365dd9a822e201bf107" parameters:nil
         success:^(AFHTTPRequestOperation *operation, id responseObject) {
             ...
             _pPopularArray = [responseObject objectForKey:@"results"];
